...
def generate_password():
    try:
        ...
        return
    # Check if user allows repetation of characters
    if repeat == 1:
    ...
    password_label = Entry(password_gen, bd=0, bg="gray85",
                           textvariable=password_v, state="readonly")
    password_label.place(x=10, y=140, height=50, width=500)
...
password_gen = Tk()
password_gen.geometry("500x500")
password_gen.title("Password Generator")
# Mention the title of the app
title_label = Label(
    password_gen, text="Password Generator", font=('Ubuntu Mono', 12))
title_label.pack()
# Read length
length_label = Label(password_gen, text="Enter the length of password: ")
